...
exports.createWorkFeesRouter = createWorkFeesRouter;
const express_1 = require("express");
function createWorkFeesRouter(db) {
    ...
    router.post('/', (req, res) => {
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `).run(yearMonth ?? '', location ?? '광양', partner ?? '', item ?? '', weightKg ?? 0, salesPrice ?? 0, salesAmount ?? 0, purchasePrice ?? 0, purchaseAmount ?? 0, memo ?? '');
    ...
    router.put('/:id', (req, res) => {
        ...
        WHERE id=?
      `).run(yearMonth ?? '', location ?? '광양', partner ?? '', item ?? '', weightKg ?? 0, salesPrice ?? 0, salesAmount ?? 0, purchasePrice ?? 0, purchaseAmount ?? 0, memo ?? '', req.params.id);
